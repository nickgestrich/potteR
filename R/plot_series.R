#' a wrapper function around tabula::plot_ford() in order to ease data preparation.
#'
...
#' @param context_order A vector giving the predefined order for the values in context_var
#'
...
plot_series <- function(data, context_var, attribute_var, context_order){
  ...
  data |>
    ...
    tidyr::pivot_wider(names_from = {{ attribute_var }}, values_from = .data$n, values_fill = list(n = 0)) |>
