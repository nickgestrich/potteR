...
```{r}
#' Coding steps:
#' 1: decide on visualisation for three possibilities: a)both numeric, b)both categorical, 
...
make_quick_plot <- function(data, x_var, y_var, ...){
  ...
  else if(is.numeric(eval(substitute(x_var), data)) && is.character(eval(substitute(y_var), data))){
...
rims_only |> make_quick_plot(rim_type, sondage)

n_distinct(rims_only$rim_angle)
...
plot_box <- function(data, catvar, numvar, ...){
  data |> 
    select({{catvar}}, {{numvar}}) |> 
    tidyr::drop_na() |> 
    ggplot(aes(x = reorder({{catvar}}, {{numvar}}), y = {{numvar}})) + 
...
rims_only |> plot_box(rim_angle, diam)
