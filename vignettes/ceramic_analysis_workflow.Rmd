...
view(dfSummary(rims))
# This would have to be wrapped in a markdown function to give out a nice report or print to pdf. One thing that needs to be changed is that character vectors are not ordered by frequency.
```
...
the shape of the vessels and one that can tell us about décor and other
...
-   the correlation coefficient (Pearson's r). The strength of the
    correlation is marked with asterisks. When there is no asterisk,
...
Again, we look at the p-value in the result. The value below 0.05 tells
us that at least one sondage is significantly different in the thickness
of its pottery. To find out how the groups compare among each other, run
the Dunnel test.
...
```{r chisquare test}
#chisq.test(all$surf, all$temper, correct=FALSE)
```
