...
```{r}
add_simple_rim <- function(data){
  
  
  
data <- data |> 
    mutate(simple_rim_type = gsub("([0-9].*)", "", rim_type))
...
filter(!if_any(everything(), is.na)) |> 
  ...
  plot <-  fviz_cluster(res.hcpc, geom = "point", main = "Factor map", axes = c(1,2)) 
...
data_clust <- data |> 
 ...
 form_groups <- list(data_clust, data_clust_s, plot, res.hcpc, res.famd)
