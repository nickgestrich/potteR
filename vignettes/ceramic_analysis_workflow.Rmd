...
```{r}
#' Coding steps:
#' 1: decide on visualisation for three possibilities: both numeric, both categorical, numeric & categorical
#' 2: place in function 
#' 3: map to all possibilities
#' 4: export pdf

# 1: visualisation for three possibilities
# a: both numeric
...
make_plot <- function(data, x_var, y_var){
  ...
  if(is.character({{x_var}}) & is.numeric({{x_var}})){
...
rims_only |> make_plot(min_th, diam)
...
rims_only |> make_scatter(min_th, diam)
...
rims_only |> plot_box(diam, rim_angle)
...
# Define a function called 'add_simple_rim' that takes a data frame as input
add_simple_rim <- function(data, column = "rim_type"){
  
  
     # Enquo the columns name
  column_name <- dplyr::ensym(column)
  
  # Inside the function, modify the input data frame
  data <- data |> 
    # Create a new column 'simple_rim_type' by applying the simplification to the specified column
...
add_simple_rim <- function(data, column = "rim_type"){
  ...
  data <- data |> 
    # Create a new column 'simple_rim_type' by applying the simplification to the specified column
    dplyr::mutate(simple_rim_type = gsub("([0-9].*)", "", {{column}}))
...
library(factoextra)
library(here)
guinea <- load("D:/Git/potteR/data/guinea.rda")
...
res.hcpc$data.clust
  fviz_cluster(geom = "point", main = "Factor map", axes = c(1,2)) 
...
res.famd <- rims |> 
tibble::column_to_rownames("unit_id") |> 
  select(diam, rim_angle, simple_rim_type) |> 
filter(!if_any(everything(), is.na)) |> 
  FAMD()
...
res.hcpc <- rims |> 
tibble::column_to_rownames("unit_id") |> 
  select(diam, rim_angle, rim_type) |> 
filter(!if_any(everything(), is.na)) |> 
  FAMD() |> 
  HCPC(graph = FALSE)  
...
rims <- add_simple_rim(rims)

library(FactoMineR)
library(factoextra)

get_first_letters <- function(input_string) {
  words <- unlist(strsplit(input_string, " ")) # Zerlegt den String in Worte
  first_letters <- substr(words, 1, 1) # Extrahiert den ersten Buchstaben jedes Wortes
  result <- paste(first_letters, collapse = "") # Fügt die ersten Buchstaben zusammen
...
data <- rims
column <- c("diam", "rim_angle", "simple_rim_type")
graph <- FALSE
...
fg <- form_groups(rims, graph = TRUE)

fg <- form_groups(rims, graph = FALSE)

fg <- form_groups(rims, column = c("diam", "rim_angle", "simple_rim_type", "min_th", "fir"), graph = TRUE)
